...
import * as photoService from './services/mockPhotoService.ts';
import * as googlePhotosService from './services/googlePhotosService.ts';

import StartScreen from './components/StartScreen.tsx';
import PhotoGrid from './components/PhotoGrid.tsx';
import SwiperView from './components/SwiperView.tsx';
import GoogleApiModal from './components/GoogleApiModal.tsx';
...
const App: React.FC = () => {
  ...
  const fetchGooglePhotos = useCallback(async () => {
    try {
        setIsLoading(true);
        setError("Lade neueste Fotos und Alben...");
        ...
        if (e.result && e.result.error) {
            ...
            errorMessage += ` (Fehler: ${errorDetails.code} - ${errorDetails.message})`;
            if (errorDetails.details && errorDetails.details[0]) {
                if (errorDetails.details[0].reason === 'SERVICE_DISABLED') {
                    errorMessage = 'Fehler: Die Google Photos Library API ist in Ihrem Google Cloud Projekt nicht aktiviert. Bitte aktivieren Sie sie und versuchen Sie es erneut.';
                }
            }
        } else if (e.message) {
        ...
        setView('start'); // Go back to start on error
    } finally {
        setIsLoading(false);
    }
  }, []);
  ...
  const connectToGoogle = useCallback(async (key: string, id: string) => {
    try {
      setIsLoading(true);
      setError("Initialisiere Google-Dienste...");
      await googlePhotosService.init(key, id, setGoogleSignedIn);
      setError("Bitte im Popup-Fenster bei Google anmelden...");
      googlePhotosService.signIn(); // Fire-and-forget, useEffect will handle the rest
    } catch(e: any) {
  ...
  const handleSelectLocalFolder = async () => {
    try {
      ...
      setIsLoading(true);
      setError("Lese Ordner & erstelle Vorschaubilder...");
  ...
  const renderContent = () => {
    if (isLoading || error) {
      return <div className="flex items-center justify-center h-full"><p className="text-center text-lg">{error || 'Lade...'}</p></div>;
    }
